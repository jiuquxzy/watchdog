...
const (
	DEFAULT_PUSH_FREQ = time.Second * 10
...
var exeConf = types.ExecConfig{
	Cmd:          []string{"cat /opt/bucket/config.yaml"},
	AttachStdout: true,
	AttachStderr: true,
	Tty:          false,
}
...
func (cli *WatchdogClient) InsertMinerInfo(cinfo status.Container) error {
	...
	conf, err := status.ParseMinerConfigFile(res)
	if err != nil {
		return errors.Wrap(err, "insert miner info error")
	}
...
func RunWatchdogClient(url string, freq time.Duration) error {
	...
	for {
		...
		if err != nil {
			log.Debugf("run watchdog client error: %s", err)
		}
		time.Sleep(freq)
...
func PostDataToServer(url string, data []MinerInfoDisplay) error {
	...
	log.Debugf("watchdog server response: %s", respData)
	return nil
}
